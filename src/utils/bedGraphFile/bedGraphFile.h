...
#include "sqlGraphFile.h"

template <typename T>
BedGraphLineStatus BedGraphFile::GetNextBedGraph (BEDGRAPH<T> &bedgraph, int &lineNum)
{
    if (_isSql) 
        return static_cast< SqlGraphFile* >(this)->GetNextBedGraph(bedgraph, lineNum);
...
template <typename T>
BedGraphLineStatus BedGraphFile::parseLine (BEDGRAPH<T> &bg, const vector<string> &lineVector, int &lineNum)
{
